...
export function UserProfilePage() {
  ...
  useEffect(() => {
    ...
    mascotStore.say(
      Array.isArray(trigger.message.text)
        ? trigger.message.text[0]
        : trigger.message.text,
      Array.isArray(trigger.message.emotion)
        ? trigger.message.emotion[0]
        : trigger.message.emotion,
    );
  }, [userId]);
  ...
  const activeTab =
    searchParams.get("tab") === "booklists" ? "booklists" : "threads";
  const booklistPage = Math.max(
    1,
    Number(searchParams.get("booklistPage")) || 1,
  );

  const setActiveTab = useCallback(
    (tab: "threads" | "booklists") => {
      setSearchParams(
        (prev) => {
          const p = new URLSearchParams(prev);
          if (tab === "threads") p.delete("tab");
          else p.set("tab", "booklists");
          p.delete("booklistPage");
          return p;
        },
        { replace: true },
      );
  ...
  const setBooklistPage = useCallback(
    (page: number) => {
      setSearchParams((prev) => {
        const p = new URLSearchParams(prev);
        if (page <= 1) p.delete("booklistPage");
        else p.set("booklistPage", String(page));
        return p;
      });
    },
    [setSearchParams],
  ...
  const booklistsQuery = useBooklistsList({
    ...
    ownerId: userId,
    sortMethod: 4,
    pageIndex: booklistPage - 1,
    pageSize: 12,
    isTournament: false,
    enabled: activeTab === "booklists" && Boolean(userId),
  });
  ...
  const threads = useMemo(
    () => (threadsQuery.data?.results || []) as Thread[],
    [threadsQuery.data?.results],
  );
  ...
  const handleShare = () => {
    ...
    setShareText(
      buildAuthorShareText({
        ...
        stats: {
          thread_count:
            profile?.stats.thread_count ?? threadsQuery.data?.total ?? 0,
  ...
  const authorContentProfile = useMemo(() => {
    ...
    threads.forEach((thread) => {
      new Set(
        (thread.tags || []).map((tag) => tag.trim()).filter(Boolean),
      ).forEach((tag) => {
    ...
    return {
      ...
      channels: Array.from(channelCounts.entries())
        .map(
          ([id, count]) =>
            [channelNameMap.get(id) || id, count] as [string, number],
        )
    ...
    <>
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col gap-10 p-4 sm:p-6 lg:gap-14 lg:p-8">
              ...
              <div className="flex w-full flex-col justify-center gap-2 sm:w-auto sm:flex-row">
                <button
                  ...
                  onClick={() => {
                    ...
                    if (activeTab === "booklists")
                      void booklistsQuery.refetch();
                  }}
        ...
        <div
          role="tablist"
          aria-label="作者内容分类"
          className="mx-auto flex items-center gap-8 border-b border-(--od-shell-line) px-3"
        >
          <button
            type="button"
            role="tab"
            ...
            className={`relative pb-3 text-sm font-semibold transition-colors after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:rounded-full after:transition-opacity ${
              activeTab === "threads"
                ? "text-(--od-accent) after:bg-(--od-accent) after:opacity-100"
                : "text-(--od-text-secondary) after:opacity-0 hover:text-(--od-accent)"
            }`}
          >
            作品
          </button>
          <button
            type="button"
            role="tab"
            aria-selected={activeTab === "booklists"}
            onClick={() => setActiveTab("booklists")}
            className={`relative pb-3 text-sm font-semibold transition-colors after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:rounded-full after:transition-opacity ${
              activeTab === "booklists"
                ? "text-(--od-accent) after:bg-(--od-accent) after:opacity-100"
                : "text-(--od-text-secondary) after:opacity-0 hover:text-(--od-accent)"
          ...
          <section className="px-1">
            ...
            <div className="mb-6 space-y-4">
              ...
              {selectedChannelIds.length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5 text-xs text-(--od-text-secondary)">
                  <span className="font-medium text-(--od-text-label)">
                    已选频道：
                  </span>
            ...
            <div className="mb-6 flex flex-wrap items-center gap-2 text-sm text-(--od-text-secondary)">
              ...
              {threadsQuery.data?.total != null && (
                <>
                  <span className="mx-1 text-(--od-text-tertiary)">·</span>
                  <span className="font-medium text-(--od-text-value)">
                    {threadsQuery.data.total}
                  </span>
                  <span>条结果</span>
                </>
              )}
          ...
          <section className="px-1">
            ...
            {booklistsQuery.isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <BookOpen className="h-10 w-10 text-(--od-text-tertiary)/35" />
                <p className="mt-4 text-sm text-(--od-text-secondary)">
                  这位作者还没有公开书单。
                </p>
              </div>
            ) : (
              <>
                ...
                {booklistTotalPages > 1 && (
                  <div className="mt-8">
                    <AnimatedPagination
                      currentPage={booklistPage}
                      totalPages={booklistTotalPages}
                      totalItems={booklistTotal}
                      onChange={setBooklistPage}
                    />
                  </div>
                )}
              </>
