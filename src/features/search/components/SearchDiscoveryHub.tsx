...
import {
  ...
  type DiscoveryRankingMetric,
} from "@/features/discovery/components/DiscoveryThreadCarousel";
import {
  CompactThreadCard,
...
const DISCOVERY_HUB_EXPANDED_KEY = "odysseia_search_discovery_expanded_v1";

function loadDiscoveryHubExpanded() {
  try {
    return window.localStorage.getItem(DISCOVERY_HUB_EXPANDED_KEY) !== "false";
  } catch {
    return true;
  }
}
...
export function SearchDiscoveryHub({
  ...
  const [activeRail, setActiveRail] =
    useState<DiscoveryRailKey>("reaction_surge");
  ...
  const applyDays = (nextDays: number) => {
    const normalizedDays = Math.max(1, Math.min(90, Math.round(nextDays)));
  ...
  return (
    <div className="mb-10 flex min-w-0 max-w-full flex-col gap-10 overflow-x-clip">
      <button
        ...
        aria-label={isExpanded ? "收起发现内容" : "展开发现内容"}
        title={isExpanded ? "收起发现内容" : "展开发现内容"}
        className="mx-auto -mb-7 inline-flex h-7 w-7 items-center justify-center rounded-full text-(--od-text-tertiary) transition-colors hover:bg-(--od-interactive-hover) hover:text-(--od-text-primary)"
      >
        {isExpanded ? (
        ...
        <>
          <section className="min-w-0 max-w-full py-6 sm:py-8">
            <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-center gap-x-6 gap-y-2 px-4 text-center">
              ...
              <details ref={daysPanelRef} className="group relative">
                <summary className="inline-flex cursor-pointer list-none items-center gap-1.5 rounded-full px-3 py-2 text-xs font-medium text-(--od-text-secondary) transition-colors hover:bg-(--od-interactive-hover) hover:text-(--od-text-primary) [&::-webkit-details-marker]:hidden">
                  <CalendarRange className="h-3.5 w-3.5" />近 {days} 天
                </summary>
            ...
            <DiscoveryThreadCarousel
              ...
              rankingMetric={activeConfig.metric}
              onOpen={onOpen}
            />
          </section>
